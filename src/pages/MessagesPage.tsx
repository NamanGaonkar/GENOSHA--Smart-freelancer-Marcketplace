...
export default function MessagesPage() {
  ...
  // Video call hooks (must be before early return)
  useEffect(() => {
  ...
  // Get the other member's profile from chat_room_members
  ...
  return (
    <div style={{ display: 'flex', gap: 0, height: 'calc(100vh - 80px)', borderRadius: 12, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--bg-card)' }}>
      {/* Room List */}
      <div
        style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {rooms.length === 0 ? (
            <div style={{ padding: 20, textAlign: 'center' }}>
              <MessageSquare size={22} style={{ margin: '0 auto 8px', opacity: 0.3 }} />
              <p style={{ color: 'var(--text-muted)', fontSize: 11 }}>No conversations yet</p>
            </div>
          ) : (
            rooms.map((room: any) => {
              const rd = room.room;
              ...
              return (
                <button key={rd.id} onClick={() => setSelectedRoom(rd.id)} style={{
                  ...
                  <div style={{ width: 30, height: 30, borderRadius: '50%', overflow: 'hidden', background: 'var(--accent-dim)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {room.other_user?.avatar_url ? (
                      <img src={room.other_user.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                      <span style={{ color: 'var(--accent)', fontSize: 11, fontWeight: 600 }}>{room.other_user?.full_name?.charAt(0) || '?'}</span>
                    )}
                  </div>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div className="gen-truncate" style={{ fontSize: 12, fontWeight: 500, color: isActive ? '#10b981' : 'var(--text)' }}>
                      {room.other_user?.full_name || rd.name || 'Chat'}
                    </div>
                    <div className="gen-truncate" style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
      ...
      {/* Chat Area */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)' }} className={!selectedRoom ? 'hidden md:flex' : 'flex'}>
        {!selectedRoom ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center' }}>
              <MessageSquare size={28} style={{ margin: '0 auto 8px', opacity: 0.2 }} />
              <p style={{ color: 'var(--text-muted)', fontSize: 12 }}>Select a conversation</p>
            </div>
          </div>
        ) : (
          <>
            {/* Header */}
            <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg-card)' }}>
              <button onClick={() => setSelectedRoom(null)} className="md:hidden" style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 2 }}>
                <ArrowLeft size={16} />
              </button>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div className="gen-truncate" style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>
                  {otherProfile?.full_name || selectedRoomData?.name || 'Chat'}
                </div>
                <div className="gen-truncate" style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 1 }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
              {messages.length === 0 && <div style={{ textAlign: 'center', padding: 30 }}><p style={{ color: 'var(--text-muted)', fontSize: 11 }}>Say hello!</p></div>}
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} style={{ display: 'flex', justifyContent: isOwn ? 'flex-end' : 'flex-start' }}>
                    <div style={{ maxWidth: '75%' }}>
                        ...
                        {msg.attachment_url && (
                          <div style={{ marginBottom: msg.content ? 6 : 0 }}>
                            {msg.attachment_name?.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                              <img src={msg.attachment_url} alt={msg.attachment_name} style={{ maxWidth: '100%', maxHeight: 150, borderRadius: 6, display: 'block' }} />
                            ) : (
                              <a href={msg.attachment_url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 8px', borderRadius: 6, background: isOwn ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.06)', textDecoration: 'none', color: isOwn ? '#fff' : 'var(--text-sec)', fontSize: 11 }}>
                                <File size={12} />
                                <span className="gen-truncate">{msg.attachment_name || 'File'}</span>
                              </a>
                            )}
                          </div>
                        )}
                      ...
                      <div style={{ fontSize: 9, color: 'var(--text-muted)', marginTop: 1, textAlign: isOwn ? 'right' : 'left' }}>
            ...
            {/* Attachment Preview */}
            {attachFile && (
              <div style={{ padding: '6px 14px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-card)' }}>
                {attachPreview ? (
                  <img src={attachPreview} alt="" style={{ width: 32, height: 32, borderRadius: 6, objectFit: 'cover' }} />
                ) : (
                  <div style={{ width: 32, height: 32, borderRadius: 6, background: 'var(--bg-input)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <File size={14} color="var(--text-muted)" />
                  </div>
                )}
                <span className="gen-truncate" style={{ flex: 1, fontSize: 11, color: 'var(--text-sec)' }}>{attachFile.name}</span>
                <button onClick={removeAttachment} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 2, display: 'flex' }}>
                  <X size={12} />
                </button>
              </div>
            )}
            ...
            {chatDisabled && (
              <div style={{ padding: '8px 14px', borderTop: '1px solid var(--border)', background: isCancelled ? 'rgba(239,68,68,0.06)' : 'rgba(16,185,129,0.06)', textAlign: 'center' }}>
                <span style={{ fontSize: 11, color: isCancelled ? '#ef4444' : '#10b981', fontWeight: 500 }}>
            ...
            <div style={{ padding: '8px 14px', borderTop: '1px solid var(--border)', background: 'var(--bg-card)' }}>
              <form onSubmit={handleSend} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <input
                  ...
                  placeholder={chatDisabled ? 'Chat is read-only for this contract' : 'Type a message...'}
